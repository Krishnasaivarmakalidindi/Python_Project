...
playing = input("▶️ Do you want to play? (yes/no): ").strip().lower()
...
for q in random_questions:
    ...
    # Take only the first character of the input if it's a letter
    answer = answer[0] if answer and answer[0].isalpha() else ''
    
    try:
        idx = ord(answer) - 65
        choice = shuffled_opts[idx]
        if choice == q["answer"]:
            print("✅ Correct!")
            score += 1
        else:
        ...
        print("✋ Invalid input. Please enter A, B, C, or D only. No points for this question.")
